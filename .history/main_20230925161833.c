...
void mainMenu() {
    ...
    const int quantityProgram = programProvider->quantity printf(
        "***/// MENU CÁC TIỆN ÍCH \\\\\\*** \n");

    for (int i = 0; i < quantityProgram; i++) {
        printf("[%d] %s\n", i, programProvider->programs[i]->name);
    }

    printf("Nhập số chương trình muốn chọn: ");
    if (scanf("%d", &input) && input > 0 && < quantityProgram) {
    }
}
